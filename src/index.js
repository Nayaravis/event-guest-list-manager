...
let listTitle = "Guest List"

titleInput.addEventListener("blur", () => {
    // prevent the user from having an empty eevnt name
    if (titleInput.value.trim() === "") {
        titleInput.value = listTitle
    } else {
        listTitle = titleInput.value.trim()
        titleInput.value = listTitle
    }
})
...
function showGuests() {
    ...
    const attending = guests.filter(guest => guest.rsvp === 'yes').length
    document.getElementById("attending-guests").textContent = attending

    const notAttending = guests.filter(guest => guest.rsvp === 'no').length
    document.getElementById("declined-guests").textContent = notAttending

    const pending = guests.filter(guest => guest.rsvp === null).length 
